'use client';
import { ActionIcon, Button, Divider, Drawer, Menu, Text, ThemeIcon } from '@mantine/core';
...
import { IconDashboard, IconLogout, IconUser, IconBoxSeam, IconPlus } from '@tabler/icons-react';
import './style.scss';
...
const NavItemsDesktop = ({ closeDrawer }: { closeDrawer: () => void }) => {
  ...
  return (
    <>
      {/* Nav Menus */}
      {NAV_ITEMS?.map((navItem, index) => <NavItem key={navItem.name + index} {...navItem} />)}
        ...
        <Menu withArrow>
          <Menu.Target>
            <Button
              variant="subtle"
              leftSection={
                <Icon className="text-lg text-indigo-500 hover:!bg-transparent" icon="iconamoon:profile-circle" />
              }
            >
...
const NavBar = () => {
  ...
  return (
    <nav className="nav-bar">
      <div className={`nav-bar-container ${pathname.startsWith('/dashboard') ? 'px-[20px]' : 'safe-area px-0'}`}>
        <Link href="/">
          <Image className="object-contain" width={137} height={30} src="/assets/icons/brand-logo.png" alt="find-us" />
        </Link>
        {/* For Desktop */}
