  ...
  return (
    <Container size="xl" className={className} style={{ padding: "2rem 0" }}>
      ...
      >
        ...
        <Grid.Col span={{ base: 12, md: 6 }} order={reverse ? 2 : 1}>
          ...
          {features.map((feature, index) => (
            <Group
              ...
              key={index}
            >
              <ThemeIcon className={`${index === 0 ? "first:!bg-[#1320AE] !text-white " : "mt-4"}`} size={40} radius="md" variant="light" color="blue">
                {feature.icon}
              </ThemeIcon>
              <div>
                <Text size="lg"  className="!font-semibold">{feature.title}</Text>
        ...
        {/* Right Column */}
        <Grid.Col span={{ base: 12, md: 6 }} order={{ md: reverse ? 1 : 2, sm: reverse ? 2 : 1 }}>
          <Image className="md:w-full md:h-full h-[200px]" src={imageUrl} alt={title} fit="cover" radius="md" />
        </Grid.Col>
      </Grid>
    </Container>
  );
