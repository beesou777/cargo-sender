...
import { EuroSenderOrder, getSingleOrderFromEuroSender } from '@/utils/euro_sender';
...
export async function GET(req: NextRequest) {
  try {
    ...
    const payments = (
      await Promise.all(
        orders.map(async (o) => (o.revolut_order_id ? await getRevolutPayment(o.revolut_order_id as string) : null)),
      )
    )
      .filter((o) => !!o)
      .reduce((acc, val) => ({ ...acc, [val.id as string]: val }), {}) as Record<string, RevolutOrderData>;

    const ordersFromEuroSender = (
      await Promise.all(orders.map((o) => getSingleOrderFromEuroSender(o.order_code as string)))
    ).reduce((acc, val) => ({ ...acc, [val.orderCode as string]: val }), {}) as Record<string, EuroSenderOrder>;

    const orderDataOutput = orders.map((o: any) => {
      o['payment'] = o.revolut_order_id ? payments[o.revolut_order_id as string] : null;
