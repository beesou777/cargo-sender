...
async function createOrder(user: CargoSenderUser | null, payload: object) {
  try {
    ...
    const revolutOrder = await createRevolutOrder(+((price - discount) * 1.5).toFixed(2) * 100, 'EUR', data.orderCode!);
...
export async function POST(req: NextRequest) {
  try {
    ...
    const result = validate === 'true' ? await validateOrder(body) : await createOrder(user, body);
