...
const serviceAccount = process.env.SERVICE_ACCOUNT_JSON
  ...
  : {
      ...
      client_email: 'firebase-adminsdk-9ll7y@cargosender-8005b.iam.gserviceaccount.com',
...
export const decodeJwtToken = async (jwtToken: string): Promise<CargoSenderUser> => {
  try {
...
export const getUser = async (req: NextRequest): Promise<CargoSenderUser> => {
  ...
  if (auth && typeof auth == 'string' && auth.split(' ').length == 2 && auth.split(' ')[0] == 'Bearer') {
